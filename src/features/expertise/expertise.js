...
export function serializeExpertiseProgress(progress) {
  return {
    ...
    legacySummary: structuredClone(
      progress.legacySummary ?? {},
    ),
...
function renderChecklistSection(
  ...
  const checkedCount = items.filter(
    (name) => selected[name] === true,
  ).length

  return `
    <section
      class="expertise-section expertise-catalog-section"
      ...
      <div class="expertise-checklist">
        ${
          items
            ...
            .join('') ||
          '<p class="metric-note">No catalog items loaded yet.</p>'
        }
      </div>
    </section>
  `
    ...
    <section class="feature-page">
      <header class="feature-header">
        <div>
          ...
          <p class="subtitle">
            Track exact proficient items. Changes save
            automatically.
          </p>
        </div>
      ...
      <section class="summary-card expertise-level-editor">
        <div>
          ...
          <p class="metric-note">
            Keep this synced with the number shown at your
            Expertise bench.
          </p>
        </div>
      ...
      <section class="panel migration-note">
        ...
        <p>
          Your old category totals are preserved while exact-item
          tracking is filled in from the generated catalog.
        </p>
      </section>
      ...
      <div class="expertise-page-grid expertise-v2-grid">
        ...
        ${renderChecklistSection(
          ...
          progress.individual?.specializations,
        )}
...
export function connectExpertiseFilters() {
  document
    .querySelectorAll('[data-catalog-search]')
    .forEach((input) => {
      input.addEventListener('input', () => {
        ...
        section
          ?.querySelectorAll('[data-search-name]')
          .forEach((item) => {
            item.hidden =
              Boolean(query) &&
...
export function readExpertiseForm(progress) {
  ...
  updated.level =
    Number(
      document.querySelector('#expertise-level-input')
        ?.value,
    ) || 0

  document
    .querySelectorAll('.expertise-item-checkbox')
    .forEach((input) => {
  ...
  document
    .querySelectorAll('[data-rank-kind]')
    .forEach((input) => {
      ...
      updated.ranks[kind][name] = Math.max(
        0,
